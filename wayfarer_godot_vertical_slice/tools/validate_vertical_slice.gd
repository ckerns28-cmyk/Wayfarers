...
func _validate_building_node(building: Node2D) -> void:
	...
	if body_shape and body_shape.shape is RectangleShape2D:
		...
		var is_proof_body: bool = building.has_method("is_proof_street_building") and building.is_proof_street_building()
		if is_proof_body:
			_expect(body_size.y >= 104.0, building.name + "_solid_building_lot_depth")
...
func _validate_proof_street(main: Node) -> void:
	...
	for id in proof_ids:
		...
		if configs_by_id.has(id):
			...
			for key in ["visual_base_anchor", "frontage_offset", "collision_rect", "y_sort_offset", "shadow_size", "lot_rect"]:
