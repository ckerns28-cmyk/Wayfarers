...
func _camera_focus_safety(main: Node, camera: Camera2D, capture: Dictionary) -> Dictionary:
	...
	for raw_id in must_fit:
		...
		var inside := view_rect.grow(-8.0).encloses(rect)
