...
const OUT_DIR := "res://artifacts/review/g420_runtime_screenshots"
const MANIFEST_FILENAME := "g420_runtime_screenshot_manifest.json"
...
const CAPTURES := [
	{
		"filename": "g420_01_hud_tavern_inn_status.png",
		"label": "HUD in Tavern/Inn district with player-facing status and quest panels",
		"target": "hud_tavern_inn_status",
	...
	{
		"filename": "g420_02_hud_commercial_avenue_readability.png",
		"label": "HUD over commercial avenue without blocking buildings, road read, or player scale",
		"target": "hud_commercial_avenue_readability",
	...
	{
		"filename": "g420_03_dialogue_prompt_fantasy_frame.png",
		"label": "Dialogue and interaction prompt use the new fantasy frame language",
		"target": "dialogue_prompt_fantasy_frame",
	...
	{
		"filename": "g420_04_metadata_expanded_review_identity.png",
		"label": "Review identity is available through the metadata toggle but not default player-facing UI",
		"target": "metadata_expanded_review_identity",
	...
	{
		"filename": "g420_05_no_hud_clean_newport.png",
		"label": "No-HUD clean Newport capture remains available for council visual review",
		"target": "no_hud_clean_newport",
	...
	{
		"filename": "g420_06_harbor_wide_hud_balance.png",
		"label": "Wider harbor view proves HUD balance against dock edge, roads, and player readability",
		"target": "harbor_wide_hud_balance",
...
func _capture_all() -> int:
	...
	_write_manifest(manifest)
	print("PASS: G-4.20 runtime screenshots -> ", ProjectSettings.globalize_path(OUT_DIR))
	return 0
...
func _capture_one(capture: Dictionary, main: Node, player: Node2D, camera: Camera2D) -> Dictionary:
	if main.has_method("set_review_screenshot_mode"):
		main.call("set_review_screenshot_mode", bool(capture.get("no_hud", false)))

	var hud := main.get_node_or_null("HUD") as CanvasLayer
	if hud != null and hud.has_method("set_review_metadata_expanded"):
		hud.call("set_review_metadata_expanded", bool(capture.get("metadata_expanded", false)))
		_set_dialogue_state(hud, bool(capture.get("dialogue_visible", false)))

	var prompt_label := player.get_node_or_null("PromptLabel") as CanvasItem
	if prompt_label != null:
		prompt_label.visible = bool(capture.get("prompt_visible", false))
	...
	var result := {
		...
		"metadata_expanded": bool(capture.get("metadata_expanded", false)),
		"dialogue_visible": bool(capture.get("dialogue_visible", false)),
...
func _set_dialogue_state(hud: CanvasLayer, visible: bool) -> void:
	...
	if dialogue_label != null:
		dialogue_label.text = "The tide is with you. The counting house opens at first bell."
...
func _base_manifest() -> Dictionary:
	return {
		"schema_id": "wayfarer.g420.runtime_screenshot_manifest.v1",
		"phase": "G-4.20",
		"status": "PENDING",
		"created_at": Time.get_datetime_string_from_system(true),
		"command": OS.get_environment("G420_CAPTURE_COMMAND"),
		...
		"required_views": [
			"hud_tavern_inn_status",
			"hud_commercial_avenue_readability",
			"dialogue_prompt_fantasy_frame",
			"metadata_expanded_review_identity",
			"no_hud_clean_newport",
			"harbor_wide_hud_balance",
		],
		"warnings": [],
		"screenshots": [],
	}
