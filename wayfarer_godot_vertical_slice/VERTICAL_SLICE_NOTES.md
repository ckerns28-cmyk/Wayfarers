...
See `WEB_DELIVERY.md`. The Godot slice is served as an additive
`/godot/*` route on the **same** Cloudflare Worker that ships the JS
site at `/`. The route is wired in
`wayfarer_v7_github_ready/worker/src/index.js` and reads files from
`wayfarer_v7_github_ready/worker/assets/godot/`. Until
`tools/export_web.sh` runs locally, a placeholder page (titled
`Build label: Godot Vertical Slice (export pending)`) is served at
`/godot/`, which is enough to confirm the route is deployed correctly.
The JS production site at `/` is unchanged.

Production preview URL: `https://wayfarers.ckerns28.workers.dev/godot/`

## Not Decided Yet

This does not decide full migration. The next review should judge the
running screenshot and hands-on movement around the five buildings
before any inventory, combat, save, or quest systems are ported.

## Remaining Risks

- The screenshots in `artifacts/screenshots/` were regenerated by the
  PIL approximation in `tools/render_slice_approx.py` because the build
  machine for this pass did not have Godot installed. The geometry,
  positions, and atlas crops are exact; the tile shading is
  approximate. Re-export from Godot locally before the next review
  snapshot.
- Cloudflare cannot build Godot exports on its own image, so the
  binaries must be exported locally and committed. Until that is done
  the live `/godot/` route shows the placeholder; the route itself is
  verifiably wired and headed.
- The `merchant_shop_house` cell in `pack_v1_a.png` is a columned
  mansion, not a literal shop-with-awning. The slice ID is preserved
  for the validator's expected-buildings list; a future content pass
  can decide whether to rename the slot or repoint it at a different
  cell.
