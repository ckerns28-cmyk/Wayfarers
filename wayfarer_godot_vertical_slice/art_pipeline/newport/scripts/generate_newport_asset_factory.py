...
def manifest_entry(asset_id: str, region: list[int], generated_paths: dict[str, Path], style_data: dict) -> dict:
    ...
    source = {
        ...
        "source_kind": "project_owned_pillow_generation_with_documented_newport_crops",
        "created_by": script,
        ...
        "license": "Wayfarer project-owned deterministic generated/composited asset; commercial project use allowed; any copied pixels are documented in-repo Newport building sprite crops; no third-party game sprites, ripped assets, marketplace/demo/sample assets, web-scraped art, copied prior prop-sheet pixels, or copied third-party pixels.",
        "license_ownership_status": "project-owned deterministic generated/composited asset; approved for normal G-4.18 review.",
        ...
        "notes": "Built with deterministic Pillow mark-making from Newport palette/style rules. Project-owned Newport building crops are used only where documented above as material/prop pixels. Local Blender components, when listed, are project-owned procedural silhouettes and are color-stylized before compositing.",
    }
    return {
        "asset_id": asset_id,
        ...
        "license_ownership_status": "project-owned deterministic generated/composited asset; documented Newport building crop pixels only; no prior-prop-sheet, third-party, marketplace, web-scraped, or ripped sprite pixels.",
        "normal_review_mode": True,
        "source_pixels_from_prior_wayfarer_assets": uses_project_owned_pixels,
        "source_pixels_from_project_owned_wayfarer_assets": uses_project_owned_pixels,
        "palette_reference_from_project_owned_wayfarer_assets": True,
        "source_pixels_from_third_party_material": False,
        "review_notes": "Passes G-4.18 Newport Visual Cohesion and Asset Provenance gates by using deterministic generated marks, Newport building palettes, and documented local procedural support assets only.",
    }
...
def write_manifest(generated_paths: dict[str, Path], style_data: dict) -> dict:
    ...
    manifest = {
        ...
        "assets": assets,
        "source_policy": "No asset enters the player-facing review build unless it passes both Newport Visual Cohesion and Asset Provenance gates.",
        ...
        "generation_policy": "Review-facing G-4.18 hero assets are deterministic project-owned Pillow output plus documented in-repo Newport material/prop crops where listed; no prior prop-sheet, third-party, marketplace, scraped, or ripped pixels are copied.",
